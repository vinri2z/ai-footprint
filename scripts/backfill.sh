...
# DEPRECATED: superseded by ingest-tokscale.sh, which is the single source of persisted usage
# across all agents (writes the `usage` table). This script still parses Claude-Code-only JSONL
# into the legacy `sessions` table and is no longer wired into setup.sh or the hooks. Kept for
# reference / legacy data only.
#
